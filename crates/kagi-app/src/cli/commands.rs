...
#[cfg(feature = "server")]
use crate::application::remote_service::{RemoteProject, RemoteService};
...
async fn handle_remote_command(
    ...
    match command {
        RemoteCommands::Login { remote, token } => {
            let result = RemoteService::new(allow_insecure)
                .login(local_data_dir()?, &remote, &token)
                .await?;
            println!(
                "{} admin token saved for server {} ({})",
                c.success("ok"),
                c.accent(&result.server_fingerprint),
                c.muted(&result.remote)
            );
        }
        RemoteCommands::Register { remote } => {
            let (base_path, _) = resolve_kagi_base()?;
            let project = RemoteProject::from_kagi_base(base_path, local_data_dir()?);
            let result = RemoteService::new(allow_insecure)
...
async fn remote_push(c: &Palette, allow_insecure: bool) -> anyhow::Result<()> {
    let (base_path, _) = resolve_kagi_base()?;
    let project = RemoteProject::from_kagi_base(base_path, local_data_dir()?);
    let result = RemoteService::new(allow_insecure).push(&project).await?;
    println!(
        "{} {} revision {}",
        c.prefix(),
        c.success("pushed"),
        c.accent(&result.revision.to_string())
    );
    Ok(())
}
...
async fn remote_status(c: &Palette, allow_insecure: bool) -> anyhow::Result<()> {
    let (base_path, _) = resolve_kagi_base()?;
    let project = RemoteProject::from_kagi_base(base_path, local_data_dir()?);
    let result = RemoteService::new(allow_insecure).status(&project).await?;
    println!(
        "{} {} local={} remote={}",
        c.prefix(),
        c.info(&result.state),
        c.accent(&result.local_revision.to_string()),
        c.accent(&result.remote_revision.to_string())
    );
    if result.pending_join_count > 0 {
        println!(
            "{} {} pending member request(s)",
            c.prefix(),
            c.warning(&result.pending_join_count.to_string())
        );
    }
    Ok(())
}
...
mod tests {
    use super::*;
    #[cfg(feature = "server")]
    use crate::application::remote_service::{
        apply_pulled_state_for_kagi_base as apply_pulled_state,
        collect_project_state_files_for_push, verify_pulled_manifest,
    };
